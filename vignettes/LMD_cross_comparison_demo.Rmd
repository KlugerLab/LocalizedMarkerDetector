...
# Loading Data
One of the downstream tasks for LMD is cross-sample comparison to identify sample-enriched or shared cell populations without the need for integration and differential abundance testing. Here we use a 10x mouse embryonic skin
dataset. The processing step can be found [here](https://github.com/ruiqi0130/LocalizedMarkerDetector/blob/main/LMD-paper/analysis/mouse_smoM2_example_process_from_10x.R).
...
```{r shared_module, fig.height=6, fig.width=6, warning = FALSE, message=FALSE}
library(ggplotify)
pl = lapply(c("Module1","Module16"), function(module_name){
  pl = lapply(c("MUT","CTL"), function(i){
    FeaturePlot(get(paste0("data_S_",i)), features = c(module_name), order = TRUE) + ggtitle(i) & labs(color = "ModuleScore") & NoAxes()
  })
...
```{r specific_module, fig.height=6, fig.width=6, warning = FALSE, message=FALSE}
