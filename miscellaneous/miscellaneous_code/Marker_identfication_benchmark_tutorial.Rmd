...
if(tissue_fullname == "Thymic"){
  options(timeout=6000)
  if (!file.exists('Human_Thymic_Epithelial_seurat_tiss.Robj')) {
     # If load url doesn't work, please download it and load from your working directory
    options(timeout=6000)
    download.file("https://datasets.cellxgene.cziscience.com/046d2e35-2ee6-4159-8db7-7b1039fe0005.rds",
                  destfile = 'Human_Thymic_Epithelial_seurat_tiss.Robj', method = 'libcurl')
  }
  load('Human_Thymic_Epithelial_seurat_tiss.Robj')
}
```

## Process
```{r}
# tiss <- readRDS("/banach1/ruiqi/local_marker/dataset/tabular_muris/marrow_facs.rds")
# DefaultAssay(tiss) <- "RNA"
# tiss = tiss %>% NormalizeData(1e6) %>%
#   ScaleData(verbose = FALSE) %>% FindVariableFeatures() %>% RunPCA(npcs = 50, verbose = FALSE)
```
...
Long Runtime Option
```{r}
# transfer the seurat obj to h5ad & process in jupyter notebook
library(reticulate)
library(anndata)
...
anndata::write_h5ad(tiss_adata, "./dataset/tm_marrow.h5ad")
```
...
df_benchmark <- tidyr::pivot_wider(df_benchmark, names_from = method, values_from = rank)
df_benchmark[is.na(df_benchmark)] = nrow(df_benchmark)

# rownames(df_benchmark) = df_benchmark$gene
# df_benchmark[order(Gene_detected_count[rownames(df_benchmark)]),"gene_count"] = 1:nrow(df_benchmark)
...
folder_path = "benchmark_result"
df_benchmark = read.table(file.path(folder_path, paste0(tissue_name,"_benchmark_rank_table.csv")))
score_measure =do.call(rbind,lapply(c(colnames(df_benchmark),"All genes"),function(method){
  if(method == "All genes"){
    ...
    top_n = c(seq(50,90,10),seq(100,1000,100))
  }
  DI = unlist(lapply(top_n, function(top){
    features = top_gs[1:top]
    # remove drop-out cells
    subtiss = subset(tiss, cells = which(colSums(subset(tiss,features = features))>0))
    subtiss <- subtiss %>% ScaleData(features = features) %>% RunPCA(features = features, npcs = 20)
    # # Silhouette score
    # d = dist(Embeddings(subtiss, reduction = "pca"))
    # c = as.integer(factor(subtiss$cell_ontology_class[labels(d)]))
  ...
  # Non-dropout Rate
  cell_count = lapply(top_n,function(n){
    colSums(dat[top_gs[1:n],]>0)
  })
  non_dropout_rate = unlist(lapply(cell_count,function(x){
    sum(x != 0)/length(x)
  }))
  
  result = data.frame(TopGene = top_n,Method = method,DensityIndex = DI, non_dropout_rate = non_dropout_rate)
  result
}) )

write.table(score_measure,file = file.path(folder_path, paste0(tissue_name,"_density_index_no_dropout.csv")),row.names = FALSE)

# error bar & null distribution
top_n = c(seq(50,90,10),seq(100,900,100),seq(1000,3000,500),seq(4000,9000,1000),seq(10000,max(df_benchmark),2000))
null_score_measure = do.call(rbind,lapply(top_n,function(top){
  set.seed(top)
...
write.table(null_score_measure,file = file.path(folder_path, paste0(tissue_name,"_DI_null.csv")),row.names = FALSE)

# smallest set of genes covering 95% cells & corresponding DI
min_coverage = 1
coverage_score_measure = do.call(rbind,lapply(colnames(df_benchmark),function(method){
  cat(method,"\n")
  top_gs = rownames(df_benchmark)[order(df_benchmark[,method])]
  min_covered = apply(dat[top_gs,] > 0, 2, function(x) which(x)[min_coverage])
  min_row_for_coverage <- as.integer(names(which(cumsum(table(min_covered)) >= ncol(dat) * 0.95)[1]))
  features = top_gs[1:min_row_for_coverage]
  
  # remove drop-out cells
  subtiss = subset(tiss, cells = which(colSums(subset(tiss,features = features))>0))
  npcs = pmin(20,length(features))
  subtiss <- subtiss %>% ScaleData(features = features) %>% RunPCA(features = features, npcs = npcs, verbose = FALSE)
  subtiss <- RunUMAP(subtiss,dims = 1:dim(subtiss@reductions$pca)[2])
  # Density-index
  if(length(features) < npcs){
    nn.dist = FNN::get.knn(t(subtiss[["RNA"]]@scale.data), k = 10, algorithm = "kd_tree")$nn.dist
    length_scale <- norm(t(subtiss[["RNA"]]@scale.data),type = "F")/sqrt(ncol(subtiss)-1)
    s2 = length_scale/mean(nn.dist)
  }else{
    nn.dist = FNN::get.knn(Embeddings(subtiss, reduction = "pca"), k = 10, algorithm = "kd_tree")$nn.dist
    sdVec <- stats::na.omit(subtiss@reductions$pca@stdev)
  ...
  data.frame(TopGene = min_row_for_coverage,Method = method,DensityIndex = s2, non_dropout_rate = sum(colSums(dat[features,]>0)!=0)/ncol(dat))
  
  DimPlot(subtiss,group.by = "cell_ontology_class") + NoLegend() + ggtitle(sprintf("%s, %d genes, DI: %.4f",method,min_row_for_coverage,s2))
}))
...
score_measure =do.call(rbind,lapply(c(colnames(df_benchmark),"All genes"),function(method){
  ...
  scores = sapply(top_n, function(top){
    features = top_gs[1:top]
    # remove drop-out cells
    subtiss = subset(tiss, cells = which(colSums(subset(tiss,features = features))>0))
    subtiss <- subtiss %>% ScaleData(features = features) %>% RunPCA(features = features, npcs = 20)
    ...
    # Silhouette score
    d = dist(Embeddings(subtiss, reduction = "pca"))
    c = as.integer(factor(subtiss$celltype[labels(d)]))
    si = mean(as.data.frame(cluster::silhouette(c, d)[,]) %>%
      group_by(cluster) %>%
      summarize(mean_silhouette = mean(sil_width)) %>%.$mean_silhouette)
    
    # NNret
    nn.result = FNN::get.knn(Embeddings(subtiss, reduction = "pca"), k = 10, algorithm = "kd_tree")
    nn.dist = nn.result$nn.dist
    nn.index = nn.result$nn.index
    nnret = mean(
      unlist(lapply(1:nrow(nn.index),function(i){
      mean(c[nn.index[i,]] == c[i])
    })) )
    
    # Density-index
    sdVec <- stats::na.omit(subtiss@reductions$pca@stdev)
    length_scale <- sqrt(sum(sdVec^2)) # length-scale * sqrt(2*(N-1)/N) = d_rms
    di = length_scale/mean(nn.dist)
    
    c(si,di,nnret)
  }) 
  scores = t(scores)
  colnames(scores) = c("Sihouette_score","DensityIndex","NNret")
  rownames(scores) = top_n
  
  # Non-dropout Rate
  cell_count = lapply(top_n,function(n){
    colSums(tiss[["RNA"]]@data[top_gs[1:n],]>0)
  })
  non_dropout_rate = unlist(lapply(cell_count,function(x){
    sum(x != 0)/length(x)
  }))
  
  result = data.frame(TopGene = top_n,Method = method, scores, non_dropout_rate = non_dropout_rate)
  result
}) )
```

# Recovery of celltypes
```{r}
# jaccard index of cell-type-specificity of each genes
cell_label = tiss$celltype
one_hot_matrix_ct <- sapply(unique(cell_label), function(x) as.integer(cell_label == x))
binary_dat = dat > apply(dat,2,median)
intersect_mtx = binary_dat %*% one_hot_matrix_ct
union_mtx = sapply(colnames(one_hot_matrix_ct),function(ct){
  rowSums((binary_dat > 0)) + colSums(one_hot_matrix_ct)[ct]
}) - intersect_mtx
jaccard_sim = intersect_mtx / union_mtx
threshold = 0.8 / ((ncol(tiss)/table(cell_label))*0.2 + 0.8)[colnames(jaccard_sim)]
recovery_mtx = t(apply(jaccard_sim,1,function(x) x > threshold))

# Prop of recovered cell types
recovery_rate =data.frame(sapply(c(colnames(df_benchmark)),function(method){
  top_gs = rownames(df_benchmark)[order(df_benchmark[,method])]
  top_n = 1:300
  scores = sapply(top_n, function(top){
    features = top_gs[1:top]
    mean(colSums(recovery_mtx[features,,drop = FALSE]) > 0)
  }) 
}) )
recovery_rate$'TopGene' = 1:nrow(recovery_rate)
```
## Plot
```{r}
recovery_rate <- reshape2::melt(recovery_rate,id.vars = "TopGene",value.name = "prop",variable.name = "Method")
recovery_rate = recovery_rate %>% filter(Method!="wilcox_no_filter")
recovery_rate$Method = ifelse(recovery_rate$Method %in% names(method_fullname),method_fullname[recovery_rate$Method],recovery_rate$Method)
recovery_rate$'color' = coldef[recovery_rate$Method]

ggplot(recovery_rate, aes(x = TopGene, y = prop, color = Method, group = Method)) + 
  geom_line() +
  scale_color_manual(values = score_measure %>% filter(Method!="All genes") %>% select(c("Method","color")) %>% distinct() %>% tibble::deframe()) + ylab("Prop. of recovered cell types")
```
...
write.table(score_df,file = file.path("marrow_result", paste0(tissue_name,"_different_knn.csv")),row.names = TRUE)
```
```{r}
# Obtain AUROC
...
write.table(auc_df,file = file.path("marrow_result", paste0(tissue_name,"_sensitivity_auroc.csv")),row.names = FALSE)
```
