...
setwd("/banach1/ruiqi/Peggy_data/Peggy_scdata/tmp")
...
# Preprocess ------------
folder_path = "/banach1/ruiqi/Peggy_data/Peggy_scdata/240329_smom2"
for(sample_name in sample_ls){
  ...
  saveRDS(data_S,file = file.path(folder_path,"process_data",sprintf("data_S_%s.rds",sample_name)))
}
rm(data_S)

# Add cell annotation with Dkk1, Dkk2, Lef1, Sox2
...
for(sample_name in sample_ls){
  assign(sprintf("data_S_%s",sample_name),
         readRDS(file.path(folder_path,"process_data",sprintf("data_S_%s.rds",sample_name))))
}
...
res = Obtain_gene_partition(dist, return_tree = TRUE)
local_gene$'gene_partition' = res$gene_partition
...
folder_path = "/banach1/ruiqi/Peggy_data/Peggy_scdata"
sample_ls = apply(expand.grid(c("E13.5", "E14.5"), c("MUT", "CTL"), stringsAsFactors = FALSE),1,function(x) paste0(x,collapse = "_"))
sample_ls = paste0("smom2_dermal_",sample_ls[c(1,3,2,4)])
for(sample_name in sample_ls){
  assign(sprintf("data_S_%s",sample_name),
...
pl_module = lapply(modules,function(module){
  g = names(local_gene$gene_partition)[local_gene$gene_partition == module][1]
...
# scatterplot
partition = paste0(sample_name,"_module")
...
long_df_rank = tidyr::pivot_longer(df_rank, 
                                   ...
                                   names_to = "Rank_Type", 
                                   values_to = "Rank")
long_df_rank$Rank_Type = as.factor(long_df_rank$Rank_Type)
ggplot(long_df_rank %>% filter(get(partition) %in% modules), aes(x=get(partition), y=Rank, color = Rank_Type)) + 
  geom_boxplot(position = position_dodge(0.75), width = 0.7) + labs(x = partition)  + 
  geom_point(position = position_jitterdodge(), size = 0.5) + 
  scale_y_continuous(breaks = c(seq(0,1000,100),seq(2000,5000,1000),seq(6000,max(long_df_rank$Rank),5000)),
                     labels = c(seq(0,1000,100),seq(2000,5000,1000),seq(6000,max(long_df_rank$Rank),5000))) + 
  scale_y_log10(breaks = 10^(0:4), labels = expression(10^0, 10^1, 10^2, 10^3, 10^4))

long_df_rank = read.csv("./smom2_merged_local_gene/local_gene_rank_filter_from_E13MUT.csv",row.names = "X")
long_df_rank$module = local_gene_smom2_dermal_E13.5_MUT$gene_partition[rownames(long_df_rank)]
long_df_rank$gene = rownames(long_df_rank)
long_df_rank = reshape2::melt(long_df_rank, value.name = "Rank", variable.name = "sample")
long_df_rank$Rank[is.na(long_df_rank$Rank)] = max(long_df_rank$Rank,na.rm = TRUE)
modules = c(18,11,12,13,19)
df = long_df_rank %>% 
  filter(module %in% modules) %>%
  filter(sample %in% c("E13CTL","E13MUT")) %>% arrange(gene, sample)
df$module = factor(df$module,levels = modules)
p_values <- unlist(lapply(levels(df$module),function(i){
  df %>% filter(module == i) %>%
    summarise(pvalue = {
      sample1_ranks <- filter(., sample == "E13MUT") %>% arrange(gene) %>% pull(Rank)
      sample2_ranks <- filter(., sample == "E13CTL") %>% arrange(gene) %>% pull(Rank)
      if(length(sample1_ranks) > 0 && length(sample2_ranks) > 0) {
        test_result <- wilcox.test(sample1_ranks, sample2_ranks, paired = TRUE)
...
# Join the p-values back with the filtered data for plotting
df <- left_join(df, data.frame(p_value = p_values,module = levels(df$module)),
                by = "module")
df$module = factor(df$module,levels = modules)
ggplot(df, aes(x=module, y=Rank, color = sample)) + 
  geom_boxplot(position = position_dodge(0.75), width = 0.7) + labs(x = "module")  + 
  geom_point(position = position_jitterdodge(), size = 0.5) +
  geom_text(aes(x = module, y = 8000, 
                    label = paste(format(p_value, digits = 2))), vjust = -0.5) + 
  ggtitle("LMD Rank") + 
  theme_minimal()

# Overlaid several modules on MUT / CTL --------
## Define which modules
sample_id = sample_ls[1]; modules = c(11:13)
sample_id = sample_ls[2]; modules = c(4:7)
...
## Visualize Jaccard Index Heatmap -------
sample_name = sample_ls[1]
data_S = get(paste0("data_S_",sample_name))
module_name = colnames(data_S@meta.data[,grep("Module",colnames(data_S@meta.data))])
jaccard_index <- 1 - philentropy::distance(
  t(data_S@meta.data[,module_name]), 
  method = "jaccard", mute.message=TRUE)
rownames(jaccard_index) = colnames(jaccard_index) = 
  gsub(findCommonPrefix(module_name),"",module_name)
jaccard_index[upper.tri(jaccard_index)] = NA
Visualize_jaccard_mtx(jaccard_index) + 
  ggtitle("ModuleActivityScore (CTR) of Three modules")

## Genes intersect heatmap ----------
my_list <- list(c(16:18,11:13), c(2:7))
names(my_list) = sample_ls[1:2]
gene_set = lapply(1:length(my_list), function(i){
  sample_id = names(my_list)[i]; modules = my_list[[i]]
  gene_ls = get(paste0("local_gene_",sample_id))$gene_partition[get(paste0("local_gene_",sample_id))$gene_partition %in% modules]
  gene_set = lapply(modules,function(i) names(gene_ls)[gene_ls==i])
  names(gene_set) = paste0(sample_id,modules)
  gene_set
})
intersection_matrix <- jaccard_index
intersection_matrix[] <- 0
flag = FALSE
for(i in 1:length(gene_set[[1]])){
  rownames(intersection_matrix)[i] = paste0(rownames(intersection_matrix)[i],
                                            "(",length(gene_set[[1]][[i]]),")")
  for(j in 1:length(gene_set[[2]])){
    if(!flag){
      colnames(intersection_matrix)[j] = paste0(colnames(intersection_matrix)[j],
                                                "(",length(gene_set[[2]][[j]]),")")
      
    }
    intersection_matrix[i,j] = length(intersect(gene_set[[1]][[i]],gene_set[[2]][[j]]))
  }
  flag = TRUE
}

## Visualize ModuleActivityScore of active cells -------
module_name = "smom2_dermal_E13.5_MUT_Module21"
...
ggplot(nn_score_df, aes(x = value, color = sample)) + 
  geom_density() + 
  labs(title = "Density Plot of avergae ModuleScore over 5 NNs", x = "average ModuleScore", y = "Density") +
  theme_minimal() + xlim(0,1)
ggplot(nn_score_df, aes(x = knn, y = value, color = sample)) + 
  geom_boxplot() + 
  labs(title = "Avergae ModuleActivityScore over kNN", y = "Average ModuleActivityScore") +
