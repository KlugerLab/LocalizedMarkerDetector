...
ggsave(filename = "fig-pipeline2.png", plot = p, width = 8, height = 5)

# Check Gene Diffusion on Cell graph by time
pl = lapply(1:length(genes),function(i){
  gene = genes[i]
  FeaturePlot_diffusion(coord = visual_space,init_state = rho[gene,],P_ls = P_ls,W = W,check_time = check_time,gene_name = gene,gene_color = scales::hue_pal()(2)[i])
})
...
df_auc = do.call(rbind,lapply(tissue_ls,function(tissue_name){df = read.table(paste0(path,tissue_name,"_auroc.csv"),header = TRUE); df$'Tissue' = tissue_name; df}))
...
fig <- ggplot(df_auc, aes(x = Tissue, y = AUROC, fill = Method)) +
  ...
  geom_text(aes(label = round(AUROC, 2)), position = position_dodge(width = 0.85), vjust = -0.25, size = 3) +
...
ggsave(filename = "fig-known_marker_comparison.png", plot = fig, width = 10, height = 5)
```
...
pl = lapply(1:length(g),function(i){
  ...
  color_label = ifelse(df_label_all[x,method_vs] > df_label_all[x,names(g)[i]],"red","blue")
...
pl[[1]] = pl[[1]] + labs(y = "Favored by LMD") + theme(axis.title.y = element_text(size = 20,face="bold", colour = "blue"))
pl[[(length(pl)/2)+1]] = pl[[(length(pl)/2)+1]] + labs(y = "Favored by Other methods") + theme(axis.title.y = element_text(size = 20,face="bold", colour = "red"))
...
pl_rank = lapply(method_vs_set,function(method){
  ...
  df_label$'color' = as.factor(df_label$'color')
  rank_cor = cor(log(df_label[,1:2],10))[1,2]
  ...
  scale_color_manual(values = c("red","blue","grey50"),
                     breaks = c(fullmethod,fullmethodvs),
...
line_type["Seurat v3 Wilcoxon unfiltered"] = "twodash"
line_type["All genes"] = "dashed"

fig = ggplot(score_measure %>% filter(Method!="All genes"), aes(x = TopGene, y = DensityIndex, color = Method, group = Method, linetype = Method),linewidth = 2) + 
  geom_line(lwd = 1) +
  ...
  scale_color_manual(values= c(coldef[names(coldef)%in%score_measure$Method],"All genes" = "black")) + 
...
ggsave(filename = "fig-densityindex.png", plot = fig, width = 8, height = 5)
```
...
## Annotate CC
```{r}
cell_cycle = read.csv("./marrow_result/annotated_cell_cycle.csv")[["Phase"]]
tiss$'Phase' = cell_cycle
p1 = DimPlot(tiss,group.by = "Phase", cols = c("grey",scales::hue_pal()(3)[2:3])) + labs(title = "Phase annotated by Seurat",color = "Phase") + theme(
  axis.title = element_blank(),
      axis.text = element_blank(),
      axis.ticks=element_blank(),
      legend.title = element_text(size = 20),
      legend.text = element_text(size = 15),
      axis.line = element_blank())
p = wrap_plots(p1,(wrap_plots(pl_module[c(16,17,18,31,32)],ncol = 3) & theme(plot.title = element_text(size = 15))) + plot_layout(guides = "collect"),nrow = 1,design = "AABBB")

ggsave(filename = "fig-module-cellcycle.png", plot = p, width = 3*5, height = 3*2)
```
## Reactome enrichment
```{r}
...
df$Description = factor(df$Description, levels = names(sort(table(df$Description))))
levels(df$Description)[6] = "HDR through Homologous Recombination\n or Single Strand Annealing (SSA)"
p = ggplot(df, aes(x=module, y=Description)) +
  geom_point(aes(size=GeneRatio, color=p.adjust), alpha=0.7) +
  scale_color_gradientn(colors=colorRampPalette(c("red", "blue"))(99), name="p.adjust") +
  theme_minimal() +
