...
figure_path = "/banach1/ruiqi/local_marker/LMD_data/figures"
dir.create(figure_path,recursive = TRUE)
setwd(figure_path)
```

# SupFig - Bone Marrow Facs Cell Embedding
```{r}
...
ggsave(filename = file.path(figure_path,"supfig-bonemarrow_facs.png"), plot = p, width = 8, height = 5, dpi = 300)
```

# SupFig - Bone Marrow droplet Cell Embedding
```{r}
dir.path <- "/banach1/ruiqi/local_marker/LMD_data"
consortium = "tabular_muris"
folder.path <- file.path(dir.path,consortium)
...
ggsave(filename = file.path(figure_path,"supfig-bonemarrow_droplet.png"), plot = p, width = 8, height = 5, dpi = 300)
```
...
consortium = "tabular_muris"
tissue_name = "marrow_facs"
folder.path = file.path(dir.path, "LocalMarkerDetector","benchmark")
```
```{r}
# Define Full name
...
consortium = "tabular_muris"
tissue_name = "marrow_facs"
df_benchmark = read.table(file.path(dir.path, "LocalMarkerDetector","benchmark", consortium, paste0(tissue_name,"_benchmark_rank_table.csv")),header = TRUE)
...
fig = fig + ggtitle(ifelse(grepl("marrow",tissue_name),"Bone Marrow",ifelse(grepl("lung",tissue_name),"Lung",ifelse(grepl("pancreas",tissue_name),"Pancreas",NA)))) +
  scale_color_manual(values = coldef) + 
  scale_linetype_manual(values = linetypedef)

# Add LowQualityRate Scatterplot
fig2 = ggplot(score_measure %>% filter(Method!="All genes") %>% filter(TopGene %in% c(50,100,500)), aes(x = LowQualityRate, y = DensityIndex, color = Method)) + 
  geom_point() + xlim(0,1) + 
  scale_color_manual(values = coldef) + facet_wrap(~TopGene)  + guides(color = "none")

(fig+fig2 + plot_layout(widths = c(1, 2))) &theme_minimal() & theme(
...
fig = wrap_plots(pl[[1]],ncol = 1) + plot_layout(guides = "collect") & theme(legend.position = 'bottom')
...
df$Top_Genes = as.factor(df$Top_Genes)
fig1 = ggplot(data = df, aes(x = pc, y = JaccardIndex, group = Top_Genes, color = Top_Genes)) +
  geom_line() +
...
fig = fig1 + fig2 + plot_annotation(title = "Sensitivity Test: Different PC", theme = theme(plot.title = element_text(size = 20)))
ggsave(filename = file.path(figure_path,"supfig-sensitivitypc.png"), plot = fig, width = 16, height = 5)
```

# SupFig - Gene-Gene Jaccard-dist HeatMap
```{r}
...
p = Visualize_gene_heatmap(dist, gene_partition = res$gene_partition_rename, gene_hree = res$gene_hree)
ggsave(filename = file.path(figure_path,"supfig-moduleheatmap.png"), plot = p, width = 10, height = 10)
```

# SupFig - Module Activity Score (Bone marrow)
```{r}
consortium = "tabular_muris"
tissue_name = "marrow_facs"
dir.path <- "/banach1/ruiqi/local_marker/LMD_data"
...
p = (wrap_plots(pl_module,ncol = 7) &  theme(plot.title = element_text(face="bold", size = 10),legend.title = element_text(size = 30),legend.text = element_text(size = 20))) + plot_layout(guides = "collect")
...
p = Visualize_jaccard_mtx(jaccard_index_pruned, threshold = thred)
...
p = (wrap_plots(pl,ncol = 6) & theme(plot.title = element_text(face="bold", size = 10),legend.title = element_text(size = 15),legend.text = element_text(size = 10))) + plot_layout(guides = "collect")
...
ggsave(filename = file.path(figure_path,"supfig-ct-specific-module1.png"), plot = p, width = 3*7, height = 3*4)
```
```{r}
# # Bipartite Plot
# jaccard_thred = 0.4
...
reduction = "tsne_cc"
reduction = "tsne"
p1 = DimPlot(tiss,group.by = "Phase", cols = c("grey",scales::hue_pal()(3)[3:2]), reduction = reduction) + labs(title = "Phase annotated by Seurat",color = "Phase") + theme(
  ...
  axis.title = element_blank(),
      ...
      legend.text = element_text(size = 15),
      axis.line = element_blank())
p2 = DimPlot(tiss_droplet,group.by = "Phase", cols = c("grey",scales::hue_pal()(3)[3:2]), reduction = reduction) + labs(title = "Phase annotated by Seurat",color = "Phase") + theme(
  axis.title = element_blank(),
      axis.text = element_blank(),
      axis.ticks=element_blank(),
      legend.title = element_text(size = 20),
...
df$Description = factor(df$Description, levels(df$Description)[rev(c(2,4,6,10,1,3,5,9,7,8))])
...
p = ggplot(df, aes(x=module, y=Description)) +
  ...
  theme(legend.position="right",
      ...
      legend.title = element_text(size = 20),
      legend.text = element_text(size = 15))
ggsave(filename = file.path(figure_path,"fig-module-cellcycle2.png"), plot = p, width = 10, height = 6)
```
...
p = wrap_plots(wrap_plots(p) & theme(axis.title = element_blank(),axis.text = element_blank(), axis.ticks=element_blank(), axis.line = element_blank()),(wrap_plots(pl,nrow = 1) &  theme(plot.title = element_text(size = 15))) + plot_layout(guides = "collect"), nrow = 1, design = "ABBBB")

ggsave(filename = file.path(figure_path,"fig-subtypeB.png"), plot = p, width = 5*3, height = 3)

ggsave(filename = file.path(figure_path,"fig-subtypeG.png"), plot = p, width = 5*3, height = 3)
```
...
ggsave(filename = file.path(figure_path, "fig-smom2_cc_module.png"), plot = p, width = 3*4 + 3, height = 3*4)
```
...
p1 = (DimPlot(data_S_merged, group.by = "type", shuffle = TRUE) & NoAxes()) + ggtitle("By Top2000 HVGs") 
p2 = (DimPlot(data_S_merged, group.by = "type", shuffle = TRUE, reduction = "umap_cc") & NoAxes()) + ggtitle("By 95 CC genes (Seurat)") 
p = p1 + p2 + plot_layout(guides = "collect")
ggsave(filename = file.path(figure_path, "supfig-smom2_E13_embed.png"), plot = p, width = 2*3+1, height = 3)
```
# Fig - Wnt Module (SmoM2)
```{r}
sample_name = sample_ls[1]
modules = module_ls[[sample_name]]
...
ggsave(filename = file.path(figure_path, "fig-smom2_wnt_module1.png"), plot = p, width = 3*3 + 3, height = 3*3)
```
## Lef1 level
```{r}
df = read.csv(file.path(dir.path,"intermediate_data","e13_smom2_wnt_level_table.csv"))
gene = "Lef1"
df$wnt_related_module = factor(df$wnt_related_module, levels = df %>% group_by(wnt_related_module) %>% summarise(Average_Expression = mean(get(gene), na.rm = TRUE)) %>% arrange(desc(Average_Expression)) %>% pull(wnt_related_module))
...
#        color = "Module") +
#   theme_minimal()
p = ggplot(df, aes(x = wnt_related_module, y = get(gene))) +
  geom_violin(scale = "width") + geom_jitter() + 
  ggsignif::geom_signif(comparisons = list(c("High-Wnt Module", "DC Module"), c("High-Wnt Module", "Wnt Module"), c("Wnt Module", "Other")),
              map_signif_level = TRUE,
              textsize = 3) + theme_minimal() +
  labs(title = paste0("Expression of ",gene), x = "", y = "Expression Level")
p = p + theme(
...
p2 = DimPlot(data_S, 
        ...
        cols = setNames(c(colorRampPalette(brewer.pal(3, "Paired"))(3),"lightgrey"),levels(data_S$'Wnt.Module'))) + ggtitle("Active Cells for Wnt-related module \n(ModuleScore > 0.4)") + NoAxes()

ggsave(filename = file.path(figure_path, "fig-smom2_wnt_module2.png"), plot = p2 + p, width = 10, height = 5)
```


# Fig - Quiescent Module (SmoM2)
```{r}
...
pl = lapply(sample_ls, function(sample_name){
  ...
  if("umap_cc_regressout" %in% names(data_S@reductions)){
    ...
    p1 = DimPlot(data_S, 
             group.by = "Phase", shuffle = TRUE, reduction = reduc) + ggtitle("") & NoAxes()
    p2 = FeaturePlot(data_S, features = modules, order = TRUE, reduction = reduc) + ggtitle("") + NoAxes() + 
    scale_color_gradient(low = "lightgrey", high = "blue", limits = c(0,1)) + labs(color = "ModuleScore")
    if(reduc == "umap"){
      cc_count = table(data_S$Phase[data_S@meta.data[,modules] > 0.5])
      p2 = p2 + labs(subtitle = sprintf("%d Active Cells \n (%d in G1, %d in G2M, %d in S)",sum(cc_count), cc_count["G1"],cc_count["G2M"],cc_count["S"]))
    }
    list(p1,p2)
  })
  }else{
    FeaturePlot(data_S, features = modules, order = TRUE, reduction = "umap") + ggtitle("") + NoAxes() + 
    scale_color_gradient(low = "lightgrey", high = "blue", limits = c(0,1)) + labs(color = "ModuleScore")
  }
})
...
ggsave(filename = file.path(figure_path, "fig-smom2_quiest_module1.png"), plot = p, width = 3*5+3, height = 2*3 + 2)
```
## Cdkn1a+ Lef1 level
```{r}
df = read.csv(file.path(dir.path,"intermediate_data","e13_smom2_cdkn1a_table.csv"))
gene = "Lef1"
df$status = factor(df$status, levels = df %>% group_by(status) %>% summarise(Average_Expression = mean(get(gene), na.rm = TRUE)) %>% arrange(desc(Average_Expression)) %>% pull(status))

p = ggplot(df, aes(x = status, y = get(gene))) +
  geom_violin(scale = "width") + geom_jitter() + 
  ggsignif::geom_signif(comparisons = list(c("Cdkn1a+", "Quiescent Module"), c("Quiescent Module", "Cdkn1a-")),
              map_signif_level = TRUE,
              textsize = 3) + theme_minimal() +
  labs(title = paste0("Expression of ",gene), x = "", y = "Expression Level")
p = p + theme(
...
```{r}
data_S = get(paste0("data_S_",sample_name))
Idents(data_S) = df$status
p2 = DimPlot(data_S, order = c("Quiescent Module","Cdkn1a+","Cdkn1a-"),
        cols = setNames(c(hue_pal()(2),"lightgrey"),c("Quiescent Module","Cdkn1a+","Cdkn1a-"))) + ggtitle("Active Cells for Quiescent cells") + NoAxes()

ggsave(filename = file.path(figure_path, "fig-smom2_quiest_module1.png"), plot = p2 + p, width = 10, height = 5)
```



# Supfig - Module-Colocalization
```{r}
df = read.csv(file = file.path(folder.path,"module_colocalize_align.csv"))
pl = lapply(unique(df$sample),function(sample_name){
  ...
  pl1 = lapply(1:nrow(df_sub),function(i){
    ...
    p2 = FeaturePlot(data_S, features = module2, order = TRUE) + ggtitle(sprintf("%s (JI: %.3f)",module_ls_ref[module2],ji))
    (p1 / p2) & scale_color_gradient(low = "lightgrey", high = "blue", limits = c(0,1)) & NoAxes() & labs(color = "ModuleScore")
  })
})
p = wrap_plots(c(pl[[1]][c(2:4,1)],pl[[2]][1],pl[[3]][c(1:3)]),nrow = 1) + plot_layout(guides = "collect")
...
msig_df = do.call(rbind,lapply(sample_ls,function(sample_name){
  ...
  df <- do.call(rbind, lapply(names(msig_result), 
                            function(i) data.frame(msig_result[[i]]@result[1:5,], module = i)))
...
msig_df$module_class = ifelse(grepl("CC",msig_df$module_name),"CC", ifelse(grepl("DC|Wnt",msig_df$module_name),"DC-related", ifelse(grepl("Quiescent",msig_df$module_name),"Quiescent",NA)))
```
```{r}
pl = lapply(c("CC","DC-related","Quiescent"),function(c){
  ggplot(msig_df %>% filter(module_class == c) %>% filter(p.adjust < 0.2), aes(x=module_name, y=Description)) +
    geom_point(aes(size=GeneRatio, color=p.adjust), alpha=0.7) +
    scale_color_gradientn(colors=colorRampPalette(c("red", "blue"))(99), name="p.adjust",limits = c(0,0.2)) + scale_size_continuous(name = "Gene Ratio", limits = c(0, 1)) + 
    labs(size="Gene Ratio",x = "Module") + facet_grid(~sample,switch = "x",scales = "free") + ggtitle(c)
})

p = (wrap_plots(pl, nrow = 1) & theme_minimal() & 
    theme(legend.position="right",
          plot.title = element_text(size = 20),
