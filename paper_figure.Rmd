...
g = c("Mogat2","Vpreb2","Cd19","Abca13","Il2rb","Mup3","Fyb","Naaa","Zf12","Phgdh")
...
pl_rank = lapply(method_vs_set,function(method){
  ...
  # library(ggrepel)
  p = p + geom_text(data = df_label %>% filter(!is.na(color)) %>% filter(label %in% g), aes(label=label))
  p
})
fig = cowplot::plot_grid(plotlist = pl_rank, ncol = length(method_vs_set))
fig
```
