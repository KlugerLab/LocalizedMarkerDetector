...
pl = lapply(1:length(genes),function(i){
  gene = genes[i]
  FeaturePlot_diffusion(coord = visual_space,init_state = rho[gene,],P_ls = P_ls,W = W,check_time = check_time,gene_name = NULL, gene_color = scales::hue_pal()(length(genes))[i])
})
...
method_ls = c("lmd","haystack","hotspot","marcopolo","semi","wilcox_default","wilcox_no_filter","hvg","gene_count")
method_fullname = setNames(c("LMD","singleCellHaystack","Hotspot","Marcopolo","SEMITONES","Seurat v4 default", "Seurat v4 Wilcoxon unfiltered","HVGs","Gene count"),method_ls)
coldef = setNames(
...
df_auc = df_auc %>% filter(gt_set %in% c("Top100","CellMarkerDB")) %>% filter(!Method %in% "HVGs")
...
path = "./benchmark_result/"
tissue_name = "marrow_facs"
score_measure = read.table(paste0(path,tissue_name,"_density_index_no_dropout.csv"),header = TRUE)
...
null_score_measure = read.csv(paste0(path,tissue_name,"_DI_null.csv"),header = TRUE)
fig = fig + geom_boxplot(data = null_score_measure, aes(x = TopGene, y = DensityIndex, group = TopGene), inherit.aes = FALSE)
...
ggplot(score_measure %>% filter(Method!="All genes") %>% filter(TopGene <= 1000), aes(x = non_dropout_rate, y = Mean_SI, color = Method)) + 
  geom_point() +
  scale_color_manual(values = score_measure %>% filter(Method!="All genes") %>% select(c("Method","color")) %>% distinct() %>% tibble::deframe()) + facet_wrap(~TopGene)
```
## Prop of recovered cts
```{r}
...
fig = ggplot(data = df, aes(x = kNN, y = JaccardIndex, group = Top_Genes, color = Top_Genes)) +
  geom_line() +
  geom_point() + labs(x = "# of kNN used", y = sprintf("Jaccard Index\n (with %s)",base),color = "Top Genes") + geom_vline(xintercept = 3, color="red", linetype="dashed") + 
