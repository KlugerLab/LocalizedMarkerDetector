...
p0 <- ggplot(local_gene$gene_table, aes(x = rank, y = score)) + geom_point() + geom_point(aes(x = length(local_gene$cut_off_gene), y = local_gene$gene_table$score[length(local_gene$cut_off_gene)]), color = "red", size = 5) +  # Add a red point at index 50
  labs(x = "Rank", y = "LMDS") + theme_minimal() + theme(
...
ggsave(filename = file.path(figure_path,"fig-subtype.png"), plot = p, width = 5*3, height = 3*2)
```
...
ggsave(filename = file.path(figure_path, "fig-smom2_quiest_module2.png"), plot = p2 / p, width = 7, height = 10)
```
