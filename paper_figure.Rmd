...
fig = cowplot::plot_grid(plotlist = pl[as.numeric(t(matrix(1:length(pl),nrow = 2)))],ncol = length(method_vs_set))
ggsave(filename = "fig-ranking_comparison.png", plot = fig, width = 25, height = 10)
```
```{r}
pl_rank = lapply(method_vs_set,function(method){
  ...
  df_label[c(which(df_label[,method] <= 100 & df_label[,method_vs] > 500)),"color"] = sprintf("Favor by %s (rank <= 100)",method)
  df_label[c(which(df_label[,method] > 500 & df_label[,method_vs] <= 100) ),"color"] = sprintf("Favor by %s (rank <= 100)",method_vs)
  df_label$'color' = as.factor(df_label$'color')
  p = ggplot(df_label, aes(x=!!as.name(method), y=!!as.name(method_vs),color = color)) + geom_point(size = 1) + 
  scale_color_manual(values = c("red", "blue", "grey50"),
                     breaks = levels(df_label$'color')) + 
    ...
    labs(x = paste0(method_fullname[method]," Rank"), y = paste0(method_fullname[method_vs]," Rank"), color = "Highlighted Genes") + theme_minimal() 
  # annotate gene labels
  # library(ggrepel)
  p = p + geom_text_repel(data = df_label %>% filter(!is.na(color)) %>% filter(label %in% g), aes(label=label),min.segment.length = unit(0, 'lines'),nudge_y = -0.5)
  p
})
pdf(width = 25,height = 5)
legend = get_legend(pl_rank[[1]] +  guides(color=guide_legend(direction="horizontal")))
prow = plot_grid(plotlist = lapply(pl_rank,function(x) x + NoLegend()), ncol = length(method_vs_set))
...
fig = ggplot(score_measure %>% filter(Method!="all genes"), aes(x = TopGene, y = DensityIndex, color = Method, group = Method)) + geom_line(aes(linetype = Method)) + guides(linetype="none") + scale_color_manual(values= coldef[names(coldef)%in%score_measure$Method]) + geom_hline(yintercept = baseline, linetype = "dashed", color = "grey1", alpha = 0.6) + annotate("text", x = max_x, y = baseline, label = "All_genes", vjust = -1, color = "grey1")  + theme_minimal() + geom_text_repel(data = score_measure %>%  filter(Method!="all genes") %>% group_by(Method) %>% slice_head(n = 2) %>% slice_tail(n = 1), aes(label = Method), hjust = 0)
...
fig = ggplot(data = average_ranks, aes(x = kNN, y = Average_Rank, group = Top_Genes)) +
  geom_line() +
  geom_point() + labs(x = "kNN", y = sprintf("Average Rank of Top # Genes\n (base on %s)",base)) + geom_vline(xintercept = 5, color="red", linetype="dashed") +
  theme_minimal() + facet_wrap(~Top_Genes,scales = "free",labeller = as_labeller(setNames(paste0("Top",unique(average_ranks$Top_Genes)),unique(average_ranks$Top_Genes)))) + NoLegend()
pdf(width = 8,height = 5)
fig
dev.off()
```
...
fig = ggplot(data = auc_df, aes(x = kNN, y = AUC), group = gt) +
  geom_line() +
  geom_point() + labs(x = "kNN", y = "AUROC") + facet_wrap(~gt, scales = "free") + 
  theme_minimal()
...
# Fig - Gene-Gene Colocolization HeatMap
```{r}
...
p = pheatmap(as.matrix(dist),
              ...
              silent = TRUE)
p = as.ggplot(p)
ggsave(filename = "fig-moduleheatmap.png", plot = p, width = 10, height = 10)
```
# Fig - Module Jaccard
```{r}
jaccard_index_pruned = jaccard_index[apply(jaccard_index,1,max) > 0.4,]
jaccard_index_pruned = jaccard_index_pruned[,apply(jaccard_index_pruned,2,max) > 0.4]

colnames(jaccard_index_pruned) = paste0("M",colnames(jaccard_index_pruned))
p = pheatmap(t(jaccard_index_pruned)[,names(sort(apply(jaccard_index_pruned,1,which.max)))],
...
p = as.ggplot(p)
ggsave(filename = "jaccard_heatmap.png", plot = p, width = 5, height = 10)
```
```{r}
# # Bipartite Plot
# jaccard_thred = 0.4
...
p = cowplot::plot_grid(prow, legend_module, rel_widths = c(10, 1))
ggsave(filename = "fig-moduletsne.png", plot = p, width = 30, height = 18)
```
