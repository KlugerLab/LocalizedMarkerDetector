...
res = LMD(dat,feature_space,max_time = 2^10,knn = 5)
...
method_ls = c("lmd","lmd_gaussian","haystack","hotspot","marcopolo","semi","wilcox_default","wilcox_no_filter","hvg")
```
```{r}
...
score_measure =do.call(rbind,lapply(c(method_ls,"All genes"),function(method){
  ...
  result = unlist(lapply(top_n, function(top){
    ...
    tiss <- tiss %>% ScaleData(features = features) %>% RunPCA(features = features, npcs = 20)
    # # Silhouette score
