...
tissue_name = "marrow_facs" #lung_facs, pancreas_facs
tiss <- readRDS(paste0(path,tissue_name,".rds"))
```
## Process
```{r}
...
write.table(max_logfc,file = paste0(path,tissue_name,"_ground_truth_c1.txt"))
```
## Criterion2: CellMarkerDB
Downloaded marker from http://bio-bigdata.hrbmu.edu.cn/CellMarker/CellMarker_download_files/file/Cell_marker_Seq.xlsx
PanglaoDB: https://panglaodb.se/markers/PanglaoDB_markers_27_Mar_2020.tsv.gz and exclude genes that reported only once
```{r}
# Load CellMarkerDB
cell_marker_db = readxl::read_xlsx("/data/ruiqi/local_marker/dataset/Cell_Marker/Cell_marker_All.xlsx") %>% filter(species == "Mouse") %>% filter(tissue_class == "Bone marrow") %>% filter(!is.na(cellontology_id)) %>% filter(!is.na(PMID)) %>% mutate(gene_name1 = marker) %>% mutate(gene_name2 = Symbol) %>% select(c("gene_name1","gene_name2")) %>% group_by(across(everything())) %>% dplyr::summarise(count = n(), .groups = "drop") %>% filter(count > 1)
...
write.table(celldb_marker,file = paste0(path,tissue_name,"_ground_truth_c2.txt"))
```
...
write.table(marker,file = sprintf("%s%s_%s.csv",path,method,tissue_name),row.names = TRUE)
```

## Seurat v3 Wilcoxon unfiltered
...
marker <- FindAllMarkers(tiss, features = rownames(dat), only.pos = FALSE, test.use = "wilcox", logfc.threshold = 0, min.pct = 0)
...
marker <- FindAllMarkers(tiss, features = rownames(dat), only.pos = FALSE, test.use = "wilcox", logfc.threshold = 0.25,min.pct = 0.1)
...
method = "marcopolo"
marker = reticulate::py$result
marker$'rank' = marker$MarcoPolo_rank
write.table(marker,file = sprintf("%s%s_%s.csv",path,method,tissue_name),row.names = TRUE)
```
...
write.table(marker,file = sprintf("%s%s_%s.csv",path,method,tissue_name),row.names = FALSE)
```
...
df_benchmark = do.call(cbind,lapply(method_ls, function(method){
  marker = read.table(sprintf("%s%s_%s.csv",path,method,tissue_name),header = TRUE)
...
write.table(df_benchmark,file = paste0(path,tissue_name,"_benchmark_rank_table.csv"),row.names = TRUE)
```
...
```
```{r}
max_logfc = read.table(paste0(path,tissue_name,"_ground_truth_c1.txt")) %>% rownames()
celldb_marker = read.table(paste0(path,tissue_name,"_ground_truth_c2.txt"))[,1]
...
levels(df$Method) = method_fullname[levels(df$Method)]
df$gt_set = factor(df$gt_set,levels = names(gt_list))
write.table(df,file = paste0(path,tissue_name,"_auroc.csv"),row.names = FALSE)
```
...
score_measure =do.call(rbind,lapply(c(method_ls,"All genes"),function(method){
  if(method == "All genes"){
    ...
    top_n = c(seq(100,900,100),seq(1000,5000,1000))
  }
...
write.table(score_measure,file = paste0(path,tissue_name,"_density_index.csv"),row.names = FALSE)
```
...
write.table(score_df,file = paste0(path,tissue_name,"_different_knn.csv"),row.names = FALSE)
```
```{r}
# Obtain AUROC
...
write.table(auc_df,file = paste0(path,tissue_name,"_sensitivity_auroc.csv"),row.names = FALSE)
```
