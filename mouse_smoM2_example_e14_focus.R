...
# Load Data --------------------
sample_ls = unlist(lapply(c("E14.5"),function(i) paste0(i,"_", c("MUT", "CTL"))))
...
# Preprocess ------------
dir.path = "/banach1/ruiqi/local_marker/LMD_data/smom2"
dir.create(file.path(dir.path,"process_data"), recursive=T)
...
# Add Cell Cycle phase annotation
...
lapply(sample_ls,function(sample_name){
  ...
  saveRDS(local_gene,file = file.path(dir.path,"intermediate_data",paste0("local_gene_",sample_name,".rds")))
})

# Group_Localized_genes ----------------
lapply(sample_ls,function(sample_name){
  data_S <- get(sprintf("data_S_%s",sample_name))
  local_gene <- readRDS(file.path(dir.path,"intermediate_data",paste0("local_gene_",sample_name,".rds")))
  ...
  saveRDS(res, file = file.path(dir.path,"intermediate_data",paste0(sample_name,"_gene_tree.rds")))
  saveRDS(dist, file = file.path(dir.path,"intermediate_data",paste0(sample_name,"_dist.rds")))
...
lapply(sample_ls,function(sample_name){
  res = readRDS(file = file.path(dir.path,"intermediate_data",paste0(sample_name,"_gene_tree.rds")))
  ...
  saveRDS(res, file.path(dir.path,"intermediate_data",paste0(sample_name,"_gene_tree.rds")))
})
...
lapply(sample_ls,function(sample_name){
  res = readRDS(file.path(dir.path, "intermediate_data",paste0(sample_name,"_gene_tree.rds")))
  local_gene = readRDS(file.path(dir.path, "intermediate_data",paste0("local_gene_",sample_name,".rds")))
