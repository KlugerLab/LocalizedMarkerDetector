...
github_packages <- c("satijalab/SeuratWrappers")
...
load(url("https://figshare.com/ndownloader/files/13092380"))
tiss <- UpdateSeuratObject(tiss)
```
We obtain the following objects from this Seurat Obj:
* `feature_space`: The output of PCA. This matrix contains the first 20 pricipal components (PCs).
...
Visualize_score_pattern(res, genes = genes, label_class = genes_label, facet_class = NULL)

# Show result
show_result_lmd(res,n = 10)
```

# Running LMD in one step
```{r}
res = LMD(dat,feature_space)
```
...
local_gene = show_result_lmd(res, n = 1000)
...
plot_grid(plotlist = pl,ncol = 10) # may takes long time to plot
```
...
jaccard_index <- apply(cell_block, 2, function(module_col) {
  apply(one_hot_matrix, 2, function(one_hot_col) {
    1 - philentropy::distance(rbind(module_col,one_hot_col), method = "jaccard")
  })
})
...
df_module[top_go_clusters,"GO_description"] = unlist(lapply(top_enrich_go,function(x) paste(x[['Description']],collapse = ",")))
df_module[top_pathway_clusters,"Pathway_description"] = unlist(lapply(top_enrich_pathway,function(x) paste(x[['Description']],collapse = ",")))
df_module[is.na(df_module)] = ""
write.csv(df_module,file = "module_description.csv",row.names = FALSE)
```
