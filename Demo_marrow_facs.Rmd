...
# abbrev[grepl("granulocytopoietic",abbrev)] = "G-poiesis"
...
p = pheatmap(t(jaccard_index_pruned),
         cluster_rows=FALSE, cluster_cols=FALSE, 
         scale = "none", 
         color = colorRampPalette(rev(c("firebrick3", "white")))(99),
         angle_col = 45,legend_title = "Jaccard Index",silent = TRUE,
         cellwidth = 15, cellheight = 10)
p_jaccard = as.ggplot(p) + coord_fixed()
ggsave(filename = "jaccard_heatmap.png", plot = p_jaccard, width = 5, height = 10)
```

### CellType-Module Matched on Cell Embeddings
...
pl <- lapply(seq(length(ct_module_match)),function(i){
    (pl_module[[ct_module_match[i]]] + NoLegend())/
    (pl_ct[[names(ct_module_match)[i]]] + NoLegend())
})

ncol = 7
legend_module = get_legend(pl_module[[1]])
legend_ct = get_legend(pl_ct[[1]])
bottom_row = plot_grid(
  plot_grid(plotlist = pl[(ncol+1):length(pl)],ncol = length(pl) - ncol), 
  plot_grid(legend_module, legend_ct, ncol = 1),
  rel_widths = c((ncol-1),1),nrow = 1)
p = plot_grid(
  plot_grid(plotlist = pl[1:ncol], ncol = ncol),
  bottom_row,
  ncol = 1)
rm(bottom_row)

ggsave(filename = "ct-specific-module.png", plot = p, width = 35, height = 20)
```
...
tiss_droplet <- UpdateSeuratObject(tiss_droplet)

# path <- "/data/ruiqi/local_marker/dataset/tabular_muris/"
# tissue_name = "marrow_droplet"
# tiss_droplet <- readRDS(sprintf("%s%s.rds",path,tissue_name))
...
```
### Check whether the modules specific in FACs still specific in droplet
```{r}
module_selected = unlist(apply(jaccard_index[rownames(jaccard_index_droplet),],1,function(x) colnames(jaccard_index)[which(x > 0.4)]))
module_selected = module_selected[order(as.numeric(module_selected))]
ct_selected = rownames(jaccard_index_pruned)[rownames(jaccard_index_pruned)%in% rownames(jaccard_index_droplet)] 
jaccard_index_pruned = jaccard_index_droplet[ct_selected,module_selected]
colnames(jaccard_index_pruned) = paste0("M",colnames(jaccard_index_pruned))
...
p = pheatmap(t(jaccard_index_pruned),
         ...
         cellwidth = 15, cellheight = 10)
p_jaccard = as.ggplot(p) + coord_fixed()
ggsave(filename = "fig-cross-dataset-jaccard.png", plot = p_jaccard, width = 5, height = 10)
```

## Save Intermediate data
```{r}
save(gene_partition,dist,gene_hree,cell_block,cell_block_droplet,jaccard_index,file = "./marrow_result/module_result.rdata")
```
...
## Module-analysis
```{r}
df_module = read.csv("./marrow_result/module_description.csv")
df_module$'multi-celltype' = NA
df_module$'note' = NA
```
...
p = as.ggplot(DimPlot(tiss,group.by = "Phase", cols = c("grey",scales::hue_pal()(3)[2:3])))

# df_module[c(16,18,17,31,32),'note'] = "cell_cycle"

p = plot_grid(plotlist = c(list(p),pl_module[c(16,18,17,31,32)]),ncol = 3)
ggsave(filename = "fig-cellcycle.png", plot = p, width = 15, height = 10)
```

### Multi-resolution Modules
```{r}
ct_prob_per_module = do.call(rbind,lapply(unique(cell_label),function(x){apply(cell_block[cell_label == x,],2,median)}))
rownames(ct_prob_per_module) = unique(cell_label)
ct_num_per_module = sort(apply(ct_prob_per_module,2,function(x)sum(x > 0.5)))

m = 19
df_module[m,"celltype"] = paste0(names(which(ct_prob_per_module[,m] > 0.5)),collapse = ",")

df_module[54,"multi-celltype"] = c("T/NK/NKT")
df_module[27,"multi-celltype"] = c("Progenitor/NK/NKT")
df_module[33,"multi-celltype"] = c("Progenitor/basophil/granulocytopoietic/half-granulocyte")
df_module[46,"multi-celltype"] = c("monocyte/granulocytopoietic/half-granulocyte")
df_module[48,"multi-celltype"] = c("monocyte/granulocytopoietic/granulocyte")
df_module[49,"multi-celltype"] = c("monocyte/macrophage/granulocytopoietic/granulocyte")
df_module[55,"multi-celltype"] = c("NK")
df_module[56,"multi-celltype"] = c("NK")
df_module[13,"multi-celltype"] = c("B")
df_module[50,"multi-celltype"] = c("monocyte/granulocytopoietic/granulocyte/basophil")
df_module[28,"multi-celltype"] = c("Progenitor")
df_module[9,"multi-celltype"] = c("immature/naive B")
df_module[11,"multi-celltype"] = c("late pro/pre B")
df_module[7,"multi-celltype"] = c("late pro/pre B")
df_module[44,"multi-celltype"] = c("granulocytopoietic/half-granulocyte")
df_module[47,"multi-celltype"] = c("granulocytopoietic/granulocyte")
df_module[52,"multi-celltype"] = c("T")
df_module[53,"multi-celltype"] = c("macrophage/NK")
df_module[1,"multi-celltype"] = c("monocyte/macrophage/granulocyte monocyte progenitor cell")
df_module[20,"multi-celltype"] = c("granulocytopoietic/Progenitor")
df_module[c(42,35),"multi-celltype"] = c("below-half-granulocyte/monocyte")
df_module[c(36,38),"multi-celltype"] = c("granulocyte monocyte progenitor cell/monocyte")
df_module[2,"multi-celltype"] = c("granulocyte monocyte progenitor cell/macrophage")
df_module[4,"celltype"] = c("granulocyte monocyte progenitor cell")
df_module[c(5,6,43),"note"] = "subtype"
```

### Joint-CT Module visualization
```{r}
level0_module = c(46,13,54,28,38)
# level1_module = c(45,41,51,24,14,4,37,3,8,10,52,56)
ncol = length(level0_module)
legend_module <- get_legend(pl_module[[1]])
prow <- cowplot::plot_grid(plotlist = lapply(pl_module[level0_module],function(x) x + NoLegend()), ncol = ncol)
p = plot_grid(prow, legend_module, rel_widths = c(ncol, .2))

ggsave(filename = "fig-joint-module.png", plot = p, width = 25, height = 5)
```
...
# 
# centroids <- aggregate(. ~ cell_label, data = as.data.frame(Embeddings(tiss, "tsne")), FUN = mean)
# p <- DimPlot(tiss, group.by = "cell_ontology_class",label = FALSE, cols = coldef) + theme_void() + NoLegend() + ggtitle("Bone Marrow FACS")
# p = p + 
#   geom_text_repel(data = centroids, aes(x = tSNE_1, y = tSNE_2, label = cell_label, color = cell_label),
#                    box.padding = 0.3, point.padding = 0.3, segment.color = 'grey50',max.overlaps = 100)

p <- DimPlot(tiss, group.by = "cell_ontology_class",label = FALSE, cols = setNames(scales::hue_pal()(2),c("late pro-B cell","granulocyte"))) + ggtitle("pre-annotated cell types")
pl_module_subtype = 
...
combined_p = plot_grid(p,plotlist = pl_module_subtype,ncol = 2) + as.ggplot(pl1) + as.ggplot(pl2) + plot_layout(design = "\nAAAABBBBBBB\nAAAACCCCCCC\n")

ggsave(filename = "fig-subtype.png", plot = combined_p, width = 35, height = 10)
```
