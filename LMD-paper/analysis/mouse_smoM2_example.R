...
source("../paths.R")
dir.path = file.path(dir.path0,"smom2")
...
lapply(sample_ls,function(sample_name){
  ...
  res = ClusterGenes(dist, clustering_method = "average", 
                              deepSplit = deepSplit, return_tree = TRUE)
  saveRDS(res, file = file.path(folder.path,paste0(sample_name,"_gene_tree.rds")))
  # VisualizeGeneHeatmap(dist, gene_partition = res$gene_partition, gene_hree = res$gene_hree)
  # VisualizeGeneTSNE(dist, gene_partition = res$gene_partition)
