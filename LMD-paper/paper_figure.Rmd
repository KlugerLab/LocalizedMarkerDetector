...
source("paths.R")
figure_path = file.path(dir.path0,"figures")
...
unique_labels <- unique(tiss$cell_ontology_class)
label_numbers <- setNames(seq_along(unique_labels), unique_labels)
tiss$cell_number <- as.character(label_numbers[tiss$cell_ontology_class])
Idents(tiss) <- tiss$cell_number 
coldef <- setNames(colorRampPalette(brewer.pal(12, "Paired"))(length(label_numbers)), 
                   label_numbers)
p1 <- DimPlot(tiss, group.by = "cell_number", label = TRUE, cols = coldef) +
  guides(color = guide_legend(ncol = 1, override.aes = list(size = 3))) +
  scale_color_manual(values = coldef, labels = paste0(seq_along(unique_labels), " - ", unique_labels)) +  # Modify legend labels
  labs(title = NULL)
...
coldef <- setNames(colorRampPalette(brewer.pal(12, "Paired"))(length(label_numbers)), 
                   label_numbers)
p2 <- DimPlot(tiss, group.by = "cell_number", label = TRUE, cols = coldef) +
  guides(color = guide_legend(ncol = 1, override.aes = list(size = 3))) +
  scale_color_manual(values = coldef, labels = paste0(seq_along(unique_labels), " - ", unique_labels)) +  # Modify legend labels
  labs(title = NULL)

p = ((p1 / p2) + plot_annotation(tag_levels = 'A')) & theme(plot.tag = element_text(size = 20))
ggsave(filename = file.path(figure_path,"supfig-bonemarrow.png"), plot = p, width = 9, height = 10, dpi = 300)
```
...
```{r}
# Plot knn graph
p = VisualizeGraph(affinity_m = W, layout = Embeddings(tiss[["tsne"]])) + NoLegend()
...
p = VisualizeScorePattern(res$score_profile/res$score_profile$max_score0, genes = genes, add_point = check_time) + guides(linetype = "none")
...
pl = lapply(1:length(genes),function(i){
  gene = genes[i]
  VisualizeDiffusion(coord = Embeddings(tiss[["tsne"]]),init_state = rho[gene,,drop = FALSE],P_ls = P_ls,W = W,check_time = check_time,gene_name = NULL, gene_color = scales::hue_pal()(length(genes))[i])
})
...
```{r}
method_ls = c("lmd","haystack","hotspot","marcopolo","semi","wilcox_no_filter","hvg", "cosg", "scmarker")
method_fullname = setNames(c("LMD","singleCellHaystack","Hotspot","Marcopolo","SEMITONES","Seurat v4","HVGs", "COSG", "SCMarker"),method_ls)
```
```{r}
dir.path <- dir.path0
# data_source = "tabular_muris"
# tissue_name = "marrow_facs"
folder.path = file.path(dir.path,"benchmark")
```
```{r}
# Define Full name
tissue_ls = list("tabular_muris" = c("marrow_facs","pancreas_facs","lung_facs","marrow_droplet"),
                 "azimuth" = c("human_kidney", "human_bone_marrow", "human_pancreas", "mouse_motor_cortex", "human_lung_hlca", "human_motor_cortex"))
data_source_full_name = c("tabular_muris" = "Tabular Muris","azimuth" = "Azimuth")
...
tissue_full_name = unlist(tissue_full_name)
tissue_full_name["human_motor_cortex"] = paste0(tissue_full_name["human_motor_cortex"],"(Human)")
tissue_full_name["mouse_motor_cortex"] = paste0(tissue_full_name["mouse_motor_cortex"],"(Mouse)")

# add technologies as suffix
techs = c(rep("FACS",3),"Droplet","10x","Cite-seq","Smart-seq2","10x","10x","Smart-seq v4")
tissue_full_name = setNames(paste0(tissue_full_name," ","(",techs,")"),names(tissue_full_name))
```
```{r}
df_auc = do.call(rbind,
                lapply(1:length(tissue_ls),function(i){
                  ...
                  df_all$'data_source' = names(tissue_ls)[i]
                  df_all}))
```
## Louvain resolution for Cluster-dependent methods
```{r}
# Plot performance for each datasets
method_subls = c("wilcox_no_filter","cosg")
df_auc_sub = do.call(rbind,lapply(method_subls, function(method){
  df = do.call(rbind,
        lapply(1:length(tissue_ls),function(i){
            df_all = NULL
            for(j in tissue_ls[[i]]){
                df = read.table(file.path(folder.path, names(tissue_ls)[i],method,paste0(j,"_auroc.csv")),header = TRUE)
                df$'Tissue' = j
                df_all = rbind(df_all,df)
            }
            df_all$'data_source' = names(tissue_ls)[i]
...
```
```{r}
# plot louvain-clustering vs. real celltype
df_tmp = do.call(rbind,
        lapply(1:length(tissue_ls),function(i){
            df_all = NULL
            for(j in tissue_ls[[i]]){
                df = read.table(file.path(folder.path, names(tissue_ls)[i],paste0(j,"_louvain_clusters.csv")),header = TRUE)
                df$'Tissue' = j
                df_all = rbind(df_all,df)
            }
            df_all$'data_source' = names(tissue_ls)[i]
            df_all}))
df_tmp$Tissue = tissue_full_name[df_tmp$Tissue]
df_tmp <- df_tmp %>%
  mutate(Tissue = factor(Tissue, levels = unique(Tissue[rev(order(data_source))])))

p = df_tmp %>% ggplot(aes(x = as.factor(res), y = ncluster, group = 1)) + 
  geom_line() + geom_hline(aes(yintercept = ncelltype), color="red", linetype="dashed") + 
    facet_wrap(~Tissue, scales = "free") + xlab("Resolution") + ggtitle("The number of Louvain clusters by different resolution")
ggsave(filename = file.path(figure_path,"supfig-seurat_performance1.png"), 
       plot = p, width = 10, height = 5)
```
```{r}
# plot Seurat
df_tmp = df_auc_sub %>% filter(gt_set %in% c("CellMarkerDB","Top100"))
df_tmp$gt_set = as.factor(df_tmp$gt_set); levels(df_tmp$gt_set) = c("Trueset1","Trueset2")
df_tmp$Tissue = tissue_full_name[df_tmp$Tissue]
df_tmp <- df_tmp %>%
  mutate(Tissue = factor(Tissue, levels = unique(Tissue[rev(order(data_source))])))

pl = lapply(method_subls,function(method){
  df_tmp %>% filter(Method == method) %>% ggplot(aes(x = as.factor(res), y = AUROC, group = gt_set, color = gt_set)) + 
  geom_line() + 
    facet_wrap(~Tissue) +
    ggtitle(method_fullname[method]) + xlab("Resolution")
})
p = wrap_plots(pl, nrow = 2)
ggsave(filename = file.path(figure_path,"supfig-seurat_performance.png"), 
       plot = p, width = 16, height = 16)
```
```{r}
# # Replace cluster-dependent method w/ resolution of maximum performance
# df_auc_sub %>% 
#   filter(gt_set %in% c("Top100", "CellMarkerDB")) %>% 
#   group_by(res) %>% 
#   summarize(median_AUROC = median(AUROC, na.rm = TRUE)) %>% 
#   arrange(desc(median_AUROC))
# 
# default_res = 5
# # df_auc_sub = df_auc_sub %>%
# #     group_by(Tissue, gt_set, data_source, Method) %>%
# #     filter(AUROC == max(AUROC))
# df_auc_sub = df_auc_sub %>% filter(res == 5) %>% select(colnames(df_auc))
# '%notin%' <- Negate('%in%')
# df_auc = rbind(df_auc %>% filter(Method %notin% method_subls),df_auc_sub)
```
## Re-organize table
```{r}
# re-organize the df
...
top_name = "Top100"
df_auc = df_auc %>% filter(gt_set %in% c("CellMarkerDB",top_name))
df_auc$Method <- with(df_auc, reorder(Method, -AUROC, FUN = median))
df_auc$gt_set <- factor(df_auc$gt_set,levels = c("CellMarkerDB",top_name))
levels(df_auc$gt_set) = c("Criterion1-\n CellMarkerDB","Criterion2-\n MaxFC")
df_auc$Tissue = tissue_full_name[df_auc$Tissue]
df_auc$data_source = data_source_full_name[df_auc$data_source]
df_auc$data_source = factor(df_auc$data_source, levels = data_source_full_name)
```
...
fig <- ggplot(df_auc, aes(x = Tissue, y = AUROC, fill = Method)) +
  geom_bar(stat = "identity", position = "dodge") + facet_grid(gt_set~data_source, scales = 'free') +
...
ranked_data <- data.frame(t(apply(wide_data[,4:ncol(wide_data)],1,function(x) rank(-x, na.last = "keep", ties.method = "min"))), check.names = FALSE)
...
df_head$DataSource = c(
  rep(df_head$DataSource[1],4),
  rep(df_head$DataSource[5],6),
  df_head$DataSource[nrow(df_head)])
...
df_long$Method = factor(df_long$Method, levels = rev(colnames(wide_data)[-c(1:3)]))
df_long$DataSource = factor(df_long$DataSource, levels = unique(df_long$DataSource))
tissue_order = unique(df_long$Tissue[order(df_long$DataSource)]); tissue_order[1:4] = tissue_order[c(2:4,1)]
df_long <- df_long %>%
...
p = ggplot(df_long, aes(x = Tissue, y = Method, fill = Rank)) +
    ...
    scale_fill_distiller(palette = "YlOrBr", direction = -1, na.value = "white", limits = c(1, max(df_long$Rank, na.rm = TRUE)), 
                         breaks = seq(1, max(df_long$Rank, na.rm = TRUE))) +
...
```
```{r}
ggsave(filename = file.path(figure_path,"fig-known_marker_comparison.png"), plot = p, width = 13, height = 8)
```
...
df_runtime$data_source = data_source_full_name[df_runtime$data_source]
```
## Figure
```{r}
coldef = setNames(
...
## Table
```{r}
df_runtime = df_runtime %>% filter(Method == "LMD") %>% arrange(Time, nCells, nGenes) %>% select(c("data_source","Tissue","nCells","ncluster","nGenes","Time")) %>% mutate(Time = round(Time, 2))
colnames(df_runtime)[grep("Time",colnames(df_runtime))] = "Time (min)"
...
```{r}
method_vs = "lmd"
method_vs_set = c("wilcox_no_filter","cosg","hotspot","marcopolo","scmarker","semi","haystack")
```
...
data_source = "tabular_muris"
tissue_name = "marrow_facs"
df_benchmark = read.table(file.path(dir.path,"benchmark", data_source, paste0(tissue_name,"_benchmark_rank_table.csv")),header = TRUE)
...
# })
# g = c("Cd19","Naaa","Mogat2","Grina","Abca13","Zf12","Il2rb","Phgdh","Vpreb2","Fyb") # <100&>500 # before revision

g = c("Hp","Cyp11a1",#wilcox
      "1810033B17Rik","Pltp",#cosg
      "Mogat2","Grina",#hotspot
      "Abca13","Zf12",#marcopolo
      "Pilra","Rgs14",#scmarker
      "Il2rb","Phgdh",#semi
      "Vpreb2","Fyb"#schaystack
      ) # <100&>500

df_label = df_label_all[g,]
df_label = df_label[order(df_label$lmd),]
colnames(df_label) = method_fullname[colnames(df_label)]
write.csv(df_label,file = file.path(dir.path,"benchmark", paste0(tissue_name,"_benchmark_rank_table_selectedgenes.csv")),row.names = TRUE)
```
### human kidney 10x
```{r}
dir.path <- dir.path0
data_source = "azimuth"
tissue_name = "human_kidney"
df_benchmark = read.table(file.path(dir.path,"benchmark", data_source, paste0(tissue_name,"_benchmark_rank_table.csv")),header = TRUE)
rownames(df_benchmark) = df_benchmark$gene
tiss = readRDS(file.path(dir.path,data_source, paste0(tissue_name,".rds")))
tiss = RunUMAP(tiss, dims = 1:20, seed.use = 42)
```
```{r}
# Plot DimPlot
...
df_label_all = df_benchmark[unique(sub),c(method_vs,method_vs_set)]
# kidney
g = c("SPINK8","SPP1",#wilcox
      "RAMP2","KIAA0101",#cosg
      "SAA2","GSTA1",#hotspot
      "MEIS2","AIM1",#scmarker
      "MUC16","ATF3",#semi
      "CEACAM6","COX7A1"#schaystack
      )
...
unified_range <- range(FetchData(tiss, vars = g), na.rm = TRUE)
...
pl = lapply(1:length(g),function(i){
  ...
  FeaturePlot(tiss,features = x,order =TRUE,pt.size = 1) + labs(color = "Expression") + scale_color_gradientn(colors = c("lightgrey", "blue"), 
                          limits = unified_range) + labs(title = x,subtitle = subtitle) + 
    theme(
...
fig = wrap_plots(pl,nrow = 2) + plot_layout(guides = "collect")

# ggsave(filename = file.path(figure_path,"fig-ranking_comparison2.png"), plot = fig, width = 35, height = 10)
ggsave(filename = file.path(figure_path,"Supfig-ranking_comparison2.png"), plot = fig, width = 32, height = 10)
```
## RankComparisonPlot
```{r}
...
# ggsave(filename = file.path(figure_path,"fig-ranking_comparison1.png"), plot = p, width = 35, height = 6)
ggsave(filename = file.path(figure_path,"Supfig-ranking_comparison1.png"), plot = p, width = 32, height = 6)
```
...
<!--   dist = CalculateGeneDistance(dat_alra[local_gene,], method = distance_method) -->
...
<!--                 silent = TRUE) -->
<!--   pl_module = CustomModulePlot(dat,visual_space,gene_partition) -->
...
null_score_measure =
  read.table(file.path(dir.path,"benchmark",data_source,paste0(tissue_name,"_DI_null.csv")),header = TRUE)
...
scmarker = score_measure %>% filter(Method == "SCMarker")
score_measure = score_measure %>%
  filter(Method == "All genes") %>% 
  slice(rep(1:n(), each = length(method_fullname))) %>% 
  mutate(Method = method_fullname) %>%
  bind_rows(score_measure) %>% filter(Method != "All genes") 
if(tissue_name == "marrow_facs"){
  score_measure = score_measure %>% filter(TopGene %in% c(100,300,500,1000,3000,5000,10000,total_gene))
}else{
  score_measure = score_measure %>% filter(TopGene %in% c(20,50,100,500,1000,5000,10000,total_gene))
}
score_measure = score_measure %>% mutate(TopGene = as.factor(TopGene)) %>% mutate(Method = factor(Method,levels = rev(method_fullname)))
levels(score_measure$TopGene)[nlevels(score_measure$TopGene)] = sprintf("All(%d)",total_gene)

numeric_levels = as.numeric(levels(score_measure$TopGene)[!grepl("All", levels(score_measure$TopGene))])
closest_level <- numeric_levels[which.min(abs(numeric_levels - scmarker$TopGene))]
score_measure = rbind(score_measure,scmarker %>% mutate(TopGene = factor(closest_level, levels = levels(score_measure$TopGene))))
levels(score_measure$Method)[grep("SCMarker",levels(score_measure$Method))] = paste0(scmarker$Method," (",scmarker$TopGene," genes)")
...
fig = ggplot(score_measure, aes(x = TopGene, y = Method, fill = DensityIndex)) + 
  ...
  ggtitle(ifelse(tissue_name == "marrow_facs", "Bone Marrow(FACS)","Bone Marrow(FACS) (granulocyte)")) + 
...
## kNN
```{r}
folder.path = file.path(dir.path,data_source,"intermediate_data", tissue_name, "sensitivity_test")
test_para = "knn"
...
fig1 = ggplot(data = df, aes(x = knn, y = JaccardIndex, group = Top_Genes, color = Top_Genes)) +
  geom_line() +
  geom_point() + labs(x = "# of kNN used", y = "Jaccard Index\n (vs. kNN5)",color = "Top Genes") + geom_vline(xintercept = "5", color="red", linetype="dashed") + 
...
fig3 = ggplot(data = df, aes(x = pc, y = JaccardIndex, group = Top_Genes, color = Top_Genes)) +
  geom_line() +
  geom_point() + labs(x = "# of PC used", y = "Jaccard Index\n (vs. 20PC)", color = "Top Genes") + geom_vline(xintercept = "20", color="red", linetype="dashed") + 
  scale_y_continuous(limits = c(0,1)) +
  theme(
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
## Rank Stability
df = read.table(file.path(folder.path,sprintf("%s_jaccard_ind_%s.csv",tissue_name,test_para)),header = TRUE)
df[, c("cellprop","rep")] = do.call(rbind,stringr::str_extract_all(df$cellprop, "(?<=_)\\d+(\\.\\d+)?"))
df$Top_Genes = as.factor(df$Top_Genes)
df_summary <- df %>%
  group_by(cellprop, Top_Genes) %>%
  summarise(
    median_Jaccard = median(JaccardIndex),
    lower = median_Jaccard - sd(JaccardIndex) / sqrt(n()),  # Standard error (SE)
    upper = median_Jaccard + sd(JaccardIndex) / sqrt(n())
  )

fig5 = ggplot(data = df_summary, aes(x = cellprop, y = median_Jaccard, group = Top_Genes, color = Top_Genes)) +
  geom_line() +
  geom_point() + 
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) + labs(x = "Cell number downsampling proprotion", y = "Jaccard Index\n (vs. Original Data)", color = "Top Genes") + geom_vline(xintercept = "20", color="red", linetype="dashed") + 
  scale_y_continuous(limits = c(0,1)) +
  theme(
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
df = df %>% filter(gt_set %in% c("Top100","CellMarkerDB"))
df$gt_set = factor(df$gt_set)
df[, c("cellprop","rep")] = do.call(rbind,stringr::str_extract_all(df$Parameter, "(?<=_)\\d+(\\.\\d+)?"))
df_summary <- df %>% select(!Parameter) %>%
  group_by(cellprop, gt_set) %>%
  summarise(
    median_auroc = median(AUROC),
    lower = median_auroc - sd(AUROC) / sqrt(n()),  # Standard error (SE)
    upper = median_auroc + sd(AUROC) / sqrt(n())
  )

fig6 = ggplot(data = df_summary, aes(x = cellprop, y = median_auroc, group = gt_set, color = gt_set)) +
  geom_line() +
  geom_point() + geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) + 
  labs(x = "Cell number downsampling proprotion", y = "AUROC", color = "Gold-Standard Genesets") + 
  scale_y_continuous(limits = c(0,1)) +
  theme(
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
fig = fig5 + fig6
ggsave(filename = file.path(figure_path,"supfig-sensitivity-umi.png"), plot = fig, width = 16, height = 5)
```
...
## Rank Stability
df = read.table(file.path(folder.path,sprintf("%s_jaccard_ind_%s.csv",tissue_name,test_para)),header = TRUE)
df[, c("umi","rep")] = do.call(rbind,stringr::str_extract_all(df$umi, "(?<=_)\\d+(\\.\\d+)?"))
df$Top_Genes = as.factor(df$Top_Genes)
df$umi = as.factor(as.numeric(df$umi))
df_summary <- df %>%
  group_by(umi, Top_Genes) %>%
  summarise(
    median_Jaccard = median(JaccardIndex),
    lower = median_Jaccard - sd(JaccardIndex) / sqrt(n()),  # Standard error (SE)
    upper = median_Jaccard + sd(JaccardIndex) / sqrt(n())
...
fig7 = ggplot(data = df_summary, aes(x = umi, y = median_Jaccard, group = Top_Genes, color = Top_Genes)) +
  geom_line() +
  geom_point() + 
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) + labs(x = "UMI downsampling proprotion", y = "Jaccard Index\n (vs. Original Data)", color = "Top Genes") + geom_vline(xintercept = "20", color="red", linetype="dashed") + 
  scale_y_continuous(limits = c(0,1)) +
  theme(
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
df = df %>% filter(gt_set %in% c("Top100","CellMarkerDB"))
df$gt_set = factor(df$gt_set)
df[, c("umi","rep")] = do.call(rbind,stringr::str_extract_all(df$Parameter, "(?<=_)\\d+(\\.\\d+)?"))
df$umi = as.factor(as.numeric(df$umi))
df_summary <- df %>% select(!Parameter) %>%
  group_by(umi, gt_set) %>%
  summarise(
...
fig8 = ggplot(data = df_summary, aes(x = umi, y = median_auroc, group = gt_set, color = gt_set)) +
  geom_line() +
  geom_point() + geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) + 
  labs(x = "UMI downsampling proprotion", y = "AUROC", color = "Gold-Standard Genesets") + 
  scale_y_continuous(limits = c(0,1)) +
  theme(
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
df = read.table(file.path(folder.path,sprintf("%s_jaccard_ind_%s.csv",tissue_name,test_para)),header = TRUE)
df$Top_Genes = as.factor(df$Top_Genes)
p1 = ggplot(data = df %>% filter(get(test_para) %in% c("binary")), 
            aes(x = Top_Genes, y = JaccardIndex, group = 1)) +
  geom_line() + labs(x = "Top Genes", y = "Jaccard Index") + ylim(0,1) + 
  theme_minimal() + ggtitle("Jaccard Index of Binary scale vs. Original scale") +
  theme(
...
p2 = ggplot(data = df %>% filter(gt_set %in% c("CellMarkerDB","Top100")) %>% mutate(Parameter = as.factor(Parameter)), 
            aes(x = gt_set, y = AUROC, group = Parameter, fill = Parameter)) +
  geom_bar(stat = "identity", position = "dodge") +  
  labs(x = "Gold-Standard Geneset", fill = "Scales") + ylim(0,1) +  
  theme_minimal() + ggtitle("AUROC of two scales") +
  theme(
    title = element_text(size = 20),
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
ggsave(filename = file.path(figure_path,"supfig-sensitivity-binary.png"), plot = p1 + p2, width = 18, height = 10)
```
...
p1 = ggplot(data = df %>% filter(get(test_para) != "kNN"), 
            aes(x = Top_Genes, y = JaccardIndex, group = graph, color = graph)) +
  geom_line() + labs(x = "Top Genes", y = "Jaccard Index", color = "Graphs") + ylim(0,1) + 
  theme_minimal() + ggtitle("Jaccard Index (vs. kNN graph)") +
  theme(
    title = element_text(size = 20),
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
p2 = ggplot(data = df %>% filter(gt_set %in% c("CellMarkerDB","Top100")) %>% mutate(Parameter = as.factor(Parameter)), 
            aes(x = gt_set, y = AUROC, group = Parameter, fill = Parameter)) +
  geom_bar(stat = "identity", position = "dodge") +  
  labs(x = "Gold-Standard Geneset", fill = "Graphs") + ylim(0,1) +  
  theme_minimal() + ggtitle("AUROC of different graphs") +
  theme(
    title = element_text(size = 20),
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
ggsave(filename = file.path(figure_path,"supfig-sensitivity-graph.png"), plot = p1 + p2, width = 18, height = 10)
```
...
## Rank Stability
df = read.table(file.path(folder.path,sprintf("jaccard_ind_%s.csv",test_para)),header = TRUE)
df$Top_Genes = as.factor(df$Top_Genes)

p1 = ggplot(data = df %>% filter(get(test_para) != "original"), 
            aes(x = Top_Genes, y = JaccardIndex, group = get(test_para), color = get(test_para))) +
  geom_line() + labs(x = "Top Genes", y = "Jaccard Index", color = "Contamination level") + ylim(0,1) + 
  theme_minimal() + ggtitle("Jaccard Index (vs. uncontaminated)") +
  theme(
    title = element_text(size = 20),
...
contaminated_genes = readRDS(file.path(dir.path,"simulated_pancreas","gene500.rds"))

# Spearman correlation of the rank of 500 contaminated genes (scatterplot)
df = reshape2::melt(score_ranked %>% mutate(gene = row.names(score_ranked)) %>% filter(gene %in% contaminated_genes),id = c("original","gene"),variable.name = "level",value.name = "rank")
cor_results <- df %>%
  group_by(level) %>%
  summarize(spearman = cor(rank, original, method = "spearman")) %>%
  mutate(label = paste0("Spearman: ", round(spearman, 4)))

p2 = ggplot(df, aes(x = original, y = rank)) +
  geom_point() + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "blue") +  
  facet_wrap(~ level) + 
  geom_text(data = cor_results, aes(x = Inf, y = Inf, label = label),
            hjust = 1.2, vjust = 1.2, size = 4, color = "red", fontface = "bold") + 
  theme_minimal() +
  labs(title = "Rank Comparison for 500 contaminated genes",
       x = "Rank (uncontaminated data)",
       y = "Rank (contaminated data)",
       color = "Contamination Level") +  
  theme_minimal() + 
  theme(
    title = element_text(size = 20),
    legend.title = element_text(size = 20),
...
ggsave(filename = file.path(figure_path,"supfig-sensitivity-ambient.png"), plot = p1 + p2 + plot_layout(widths = c(1, 2)), width = 18, height = 6)
```
...
df = read.table(file.path(folder.path,sprintf("jaccard_ind_%s.csv",test_para)),header = TRUE)
df$Top_Genes = as.factor(df$Top_Genes)
df$Ncell = as.factor(df$Ncell)

p1 = ggplot(data = df, aes(x = Ncell, y = JaccardIndex, group = Top_Genes, color = Top_Genes)) +
  geom_line() + geom_point() + labs(x = "Ncell for coarse-grained cell graph", y = "Jaccard Index\n (vs. Original graph)", color = "Top Genes") + geom_vline(xintercept = "30651", color="red", linetype="dashed") + 
  scale_y_continuous(limits = c(0,1)) +
  theme(
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
# AUC stability
df = read.table(file.path(folder.path,sprintf("auroc_%s.csv",test_para)),header = TRUE)
df = df %>% filter(gt_set %in% c("Top100","CellMarkerDB"))
df$Parameter = as.factor(df$Parameter)
df$gt_set = factor(df$gt_set)

p2 = ggplot(data = df, aes(x = Parameter, y = AUROC, group = gt_set, color = gt_set)) +
  geom_line() +
  geom_point()  + 
  labs(x = "Ncell for coarse-grained cell graph", y = "AUROC", color = "Gold-Standard Genesets") + 
  scale_y_continuous(limits = c(0,1)) +
  theme(
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
...
df = read.table(file.path(folder.path,sprintf("runtime_%s.csv",test_para)),header = TRUE)
df$nCells = as.factor(df$nCells)
p3 = ggplot(data = df, aes(x = nCells, y = Time_min, group = 1)) + geom_line() +
  geom_point()  + 
  labs(x = "Ncell for coarse-grained cell graph", y = "RunTime (min)") +
  theme(
    legend.title = element_text(size = 20),
    legend.text = element_text(size = 15),
    axis.title.x = element_text(size = 20),
...
p = ((p1 + p2) / (p3 + plot_spacer())) + plot_annotation(tag_level = 'A') & theme(plot.tag = element_text(face="bold", size = 20)) 
ggsave(filename = file.path(figure_path,"supfig-sensitivity-cg.png"), plot = p, width = 16, height = 10)
```

## Overall Plot
```{r}
p = ((fig1 + fig2) / (fig3 + fig4) / (fig5 + NoLegend() + fig6 + NoLegend()) / (fig7+ NoLegend() + fig8+ NoLegend())) + plot_layout(guides = "collect") + plot_annotation(tag_level = 'A') & theme(plot.tag = element_text(face="bold", size = 20)) 
ggsave(filename = file.path(figure_path,"supfig-sensitivity.png"), plot = p, width = 16, height = 20)
```

## Cell Type Unbalanced/Connectivity
```{r}
data_source = "tabular_muris"
tissue_name = "marrow_facs"
dir.path <- dir.path0
...
df_benchmark = read.table(file.path(dir.path,"benchmark", data_source, paste0(tissue_name,"_benchmark_rank_table.csv")),header = TRUE)
rownames(df_benchmark) = df_benchmark$gene
df_benchmark = df_benchmark[order(df_benchmark$lmd),]
celldb_marker = read.table(file.path(folder.path,"ground_truth_geneset", paste0(tissue_name,"_ground_truth_c2.txt")))[,1]
max_logfc = read.table(file.path(folder.path,"ground_truth_geneset", paste0(tissue_name,"_ground_truth_c1.txt"))) %>% rownames()

dat_binary = tiss@assays$RNA@counts > 0

gene_list = lapply(c(seq(100,2000,100),1277),function(x)df_benchmark$gene[1:x])
names(gene_list) = paste0("lmd_top_",c(seq(100,2000,100),1277))
max_logfc_list = lapply(c(seq(100,2000,100),1277),function(x) max_logfc[1:x])
names(max_logfc_list) = paste0("maxfc_top_",c(seq(100,2000,100),1277))
gene_list = c(gene_list,max_logfc_list,list("CellMarkerDB_1277_genes" = celldb_marker))

cell_label = tiss$cell_ontology_class
coldef = setNames(colorRampPalette(brewer.pal(12, "Paired"))(length(unique(cell_label))),unique(cell_label))

df_prop = do.call(cbind,lapply(names(gene_list),function(top_gene){
  genes = gene_list[[top_gene]]
  nFeatures = colSums(dat_binary)
  nFeatures_lmd = colSums(dat_binary[genes,])
  prop = nFeatures_lmd/length(genes)
  prop
}))
colnames(df_prop) = names(gene_list)
tiss = AddMetaData(tiss,data.frame(df_prop))

pl <- lapply(colnames(df_prop),function(top_gene){
  # FeaturePlot(tiss, features = top_gene, order = TRUE) + scale_color_gradient(low = "lightgrey", high = "blue", limits = c(0,1))
  Idents(tiss) = tiss$cell_ontology_class
  VlnPlot(tiss, features = top_gene, cols = coldef) + ylim(0,1)
})
p = wrap_plots(pl, nrow = 5) & NoAxes()
ggsave(filename = "1.png",p,width = 15,height = 15)

p = (wrap_plots(pl, nrow = 5) & theme(legend.position = "bottom",axis.text.x = element_blank()) ) + plot_layout(guides = "collect")
ggsave(filename = "1.png",p,width = 28, height = 12)

VlnPlot(tiss, features = c("lmd_top_1300", "CellMarkerDB_1277_genes"), 
        group.by = "cell_ontology_class", 
        stack = TRUE) + xlab("Proportion") + 
...
```{r}
p1 = VisualizeGeneHeatmap(dist, gene_partition = res$gene_partition_rename, gene_hree = res$gene_hree)
```
...
pl_ct = lapply(colnames(one_hot_matrix),function(ct){
  CustomFeaturePlot(coord = Embeddings(tiss[["tsne"]]),value = as.character(one_hot_matrix[,ct]), title_name = ct, value_name = "Indicator") + scale_color_manual(values = c("0" = "lightgrey", "1" = scales::hue_pal()(1)))
})
...
data_source = "smom2"
dir.path = dir.path0
dir.path = file.path(dir.path,data_source)
